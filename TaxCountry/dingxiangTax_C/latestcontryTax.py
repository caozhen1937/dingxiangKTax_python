...
#存储图片
def saveInterpretationWebSitePageURL(path,driver,url):
    #print("开始----"+str(id))
    driver.set_window_size(800, 100)
    #设置页面加载超时
    ...
    while True:
        ...
        if PageNumaber==0:
            break
        nextpagejs="pageName="+str(PageNumaber)+";goPub('../../n810341/n810755/index_2420064_"+str(PageNumaber)+".html')"
...
def saveInterpretationWebSitePageContent(driver,path):
    ...
    # 爬虫开始
        ...
        #老快照
        fileBlock='latestcountryimage\\'+str(urlItem).replace('../..','').split("/")[3].replace('c',"")+'_block.png'
        id=str(urlItem).replace('../..','').split("/")[3].replace('c',"")
        #新快照
        fileImage='latestcountryimage\\'+str(urlItem).replace('../..','').split("/")[3].replace('c',"")+'.png'
        ...
        if (os.path.exists(fileBlock)):
            ...
            if (equal(fileBlock, fileImage)):
                ...
                # 标题
                countrytaxtitle = driver.title
                # 内容
                countrytaxcontent = driver.find_element_by_xpath("//*[@id='tax_content']").get_attribute('innerHTML')
                # print("内容" + Interpretationcontent)
                # 文号
                countrytaxlocation = driver.find_element_by_xpath("/html/body/div[6]/ul[1]/li[3]").text
                # print("官网位置"+Interpretationurl)
                # 官方链接地址
                countrytaxurl = str(urlItem).replace('../..', 'http://www.chinatax.gov.cn')
                # 发文单位
                countrytaxinstitutions = driver.find_element_by_xpath("/html/body/div[6]/ul[1]/li[1]").text

                updateMysql(id,countrytaxtitle, countrytaxcontent, countrytaxlocation, countrytaxinstitutions,countrytaxurl)
            ...
            try:
                ...
                countrytaxlocation = driver.find_element_by_xpath("/html/body/div[7]/ul[1]/li[3]").text
            #print("官网位置"+Interpretationurl)
            #官方链接地址
            countrytaxurl=str(urlItem).replace('../..','http://www.chinatax.gov.cn')
            #发文单位
            try:
                ...
                countrytaxinstitutions = driver.find_element_by_xpath("/html/body/div[7]/ul[1]/li[1]").text
            #print(driver.page_source)
            saveToMysql(id,countrytaxtitle, countrytaxcontent, countrytaxlocation, countrytaxinstitutions,countrytaxurl)
        time.sleep(2)

#写入数据库
def saveToMysql(id,countrytaxtitle, countrytaxcontent, countrytaxlocation, countrytaxinstitutions,countrytaxurl):
    # 标题 countrytaxtitle
    # 内容 countrytaxcontent
    # 文号 countrytaxlocation
    # 发文单位 countrytaxinstitutions
    # 官方地址 countrytaxurl
    ...
    cur = conn.cursor()
    countrytaxtitle = str(countrytaxtitle).encode('utf-8').decode()
    countrytaxcontent = str(countrytaxcontent).encode('utf-8').decode().replace("'","^")
    countrytaxlocation = str(countrytaxlocation).encode('utf-8').decode()
    countrytaxinstitutions = str(countrytaxinstitutions).encode('utf-8').decode()
    countrytaxurl = str(countrytaxurl).encode('utf-8').decode()

    sql = u"INSERT INTO `latestpagecountrytax` (`id`,`countrytaxtitle`, `countrytaxcontent`, `countrytaxlocation`, `countrytaxinstitutions`, `countrytaxurl`) VALUES ('"+id+"','"+countrytaxtitle+"','"+countrytaxcontent+"', '"+countrytaxlocation+"', '"+countrytaxinstitutions+"', '"+countrytaxurl+"')"
    sta = cur.execute(sql)
    if sta == 1:
        print('Done')
    else:
...
def updateMysql(id,countrytaxtitle, countrytaxcontent, countrytaxlocation, countrytaxinstitutions,countrytaxurl):
    print("更新mysql数据")
    # 标题 countrytaxtitle
    # 内容 countrytaxcontent
    # 文号 countrytaxlocation
    ...
    sql=u"UPDATE `latestpagecountrytax` SET (`countrytaxtitle`='"+countrytaxtitle+"',`Interpretationtime`='"+countrytaxcontent\
        +"',`countrytaxlocation`='"+countrytaxlocation+"',`countrytaxinstitutions`='"+countrytaxinstitutions+"',`countrytaxurl`='"+countrytaxurl+"') WHERE (`id`="+id+")"
    sta = cur.execute(sql)
    conn.commit()
    if sta == 1:
        print('Done')
...
def crawlerMain():
    ...
    #目标url地址
    url = "http://www.chinatax.gov.cn/n810341/n810755/index.html"
    path = "latestcountryurl.txt"
    #获取url  政策解读
    # print("----清空"+path+"-----")
    # with open(path, 'w') as f:
    #     print("清空文件")
    #saveInterpretationWebSitePageURL(path,driver, url)
